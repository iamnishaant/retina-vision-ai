...
// import { Upload as UploadIcon, Image as ImageIcon, X, ArrowRight, Loader2 } from 'lucide-react';
...
//       const resp = await fetch('http://127.0.0.1:8000/preprocess', {
//         method: 'POST',
//         body: form,
//       });
...
//       // map expected names -> state setters
//       const mapping: Array<[string, (v: string | null) => void]> = [
//         ['1_original.jpg', (v) => setPreview(v)],
//         ['2_cropped.jpg', (v) => setCroppedSrc(v)],
//         ['3_resized.jpg', (v) => setResizedSrc(v)],
//         ['4_clahe.jpg', (v) => setEqualizedSrc(v)],
//         ['5_vessel.jpg', (v) => setVesselMaskSrc(v)],
//       ];

//       for (const [name, setter] of mapping) {
...
//       toast.success('Server preprocessing complete — images loaded');
//     } catch (err) {
//       console.error('Upload to server failed', err);
//       toast.error('Server preprocessing failed — see console');
//     } finally {
...
//                       disabled={isAnalyzing}
//                       className="px-4 py-3 bg-emerald-600 text-white rounded-xl disabled:opacity-50"
//                     >
//                       Send to Server
//                     </button>
...
const Upload = () => {
  ...
  async function runPreprocessing(dataUrl: string) {
    ...
    // Preserve original colors in client-side preprocessing — no channel equalization
    let equalized: HTMLCanvasElement = cropped.canvas;
  ...
  // No-op equalization kept intentionally to preserve original colors on the client.
  function equalizeLuminance(canvas: HTMLCanvasElement, w: number, h: number) {
    return canvas;
  }
